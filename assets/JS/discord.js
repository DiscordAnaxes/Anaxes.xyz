...
fetch(url).then(function (response) {
    if (response.ok) {
      ...
      response.json().then(function (data) {
        ...
        var status = `${
          data.data.active_on_discord_desktop ? data.data.activities[0].emoji.name + ` ${data.data.activities[0].state.split("", 20).reduce((o, c) => (o.length === 19 ? `${o}${c}...` : `${o}${c}`),"")}` : "Anaxes#3274"
        }`;
