    ...
    .then(function(response) {
          if(response.ok) { // Check if response went through
              response.json().then(function(data) { 
                  ...
                  var spotify = `${data.data.listening_to_spotify ? `<div class="flex-shrink-0 flex items-center"><i class="fab fa-spotify text-green-500"></i>&nbsp;Listening to&nbsp;<a href="https://open.spotify.com/track/${data.data.spotify.track_id}" target="_blank" class="hover:text-green-500 flex-shrink-0 flex items-center"><img src=${data.data.spotify.album_art_url} class="block h-5 w-auto rounded-full" />&nbsp;<span class="font-bold">${data.data.spotify.song.split('', 20).reduce((o, c) => o.length === 19 ? `${o}${c}...` : `${o}${c}` , '')}</span>&nbsp;by&nbsp;<span class="font-bold">${data.data.spotify.artist.split('', 10).reduce((o, c) => o.length === 9 ? `${o}${c}...` : `${o}${c}` , '')}</span>&nbsp;<i class="fad fa-external-link"></i></a></div>` : `<p><i class="twa twa-satellite"></i>&nbsp;Status:&nbsp;<span class="font-bold">${data.data.activities[0].emoji.name}&nbsp;${data.data.activities[0].state}</span></p>`}`;
