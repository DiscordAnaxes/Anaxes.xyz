...
fetch(url).then(function (response) {
    if (response.ok) {
        ...
        response.json().then(function (data) {
            ...
            var spotify = `${data.data.listening_to_spotify
                ? `<div class="flex-shrink-0 flex items-center"><i class="fab fa-spotify text-green-500"></i>&nbsp;Listening to&nbsp;<a href="https://open.spotify.com/track/${data.data.spotify.track_id}" target="_blank" class="hover:text-green-500 flex-shrink-0 flex items-center"><img src=${data.data.spotify.album_art_url} class="block h-5 w-auto rounded-full" />&nbsp;<span class="font-bold">${data.data.spotify.song
                    .split("", 20)
                    .reduce((o, c) => (o.length === 19 ? `${o}${c}...` : `${o}${c}`), "")}</span></a>&nbsp;by&nbsp;<a href="https://open.spotify.com/search/${data.data.spotify.artist.split(";")[0]}" target="_blank" class="hover:text-green-500"><span class="font-bold">${data.data.spotify.artist
                    .split("", 10)
                    ...
                    : data.data.activities[1].name === "Visual Studio Code"
                        ...
                        : data.data.activities[1].name === "Crunchyroll"
                            ? `<p><i class="fad fa-video text-orange-500"></i> Crunchyroll:&nbsp;${data.data.activities[1].details
                                ...
                                .split("", 30)
                                .reduce((o, c) => (o.length === 29 ? `${o}${c}...` : `${o}${c}`), "")}</p>`
...
fetch(url).then(function (response) {
    if (response.ok) {
        ...
        response.json().then(function (data) {
            ...
            var status = `${data.data.discord_status === "online"
        ...
        response.json().then(function (data) {
			...
			<div class="flex items-center px-6 py-4 transform hover:scale-105 duration-200 shadow rounded-md px-6 sm:px-4 hover:bg-gray-100 cursor-pointer opacity" style="background-color: #0e141d; border-radius: 1vw; ${data.data.spotify && data.data.activities[1].name === "Spotify" ? `background-image: url(${data.data.spotify.album_art_url}); background-position: center; ` : ""}>
			  <div class="flex items-center">
				<img draggable="false" class="h-16 w-16 rounded-md" src=${data.data.listening_to_spotify && data.data.activities[1].name === "Spotify"
				...
				<p class="ml-4 text-sm flex flex-col justify-between leading-snug">
					<span class="font-bold">${data.data.listening_to_spotify && data.data.activities[1].name === "Spotify"
                ? "Listening&nbsp;to&nbsp;<i class='fab fa-spotify text-green-500'></i>"
                : `${data.data.activities[1].name === "Visual Studio Code" ? "Playing&nbsp;<i class='fad fa-code text-blue-500'></i>" : `${data.data.activities[1].name === "GitHub" ? "Playing&nbsp<i class='fab fa-github'></i>" : "Playing"}`}`}&nbsp;${data.data.activities[1].name}</span>
					<span class="opacity-75">${data.data.listening_to_spotify && data.data.activities[1].name === "Spotify"
                ? `🎶 ${data.data.activities[1].details.split("", 25).reduce((o, c) => (o.length === 24 ? `${o}${c}...` : `${o}${c}`), "")}</span>`
                : `${data.data.activities[1].details.split("", 25).reduce((o, c) => (o.length === 24 ? `${o}${c}...` : `${o}${c}`), "")}</span>`}
					${data.data.listening_to_spotify && data.data.activities[1].name === "Spotify"
                ? data.data.activities[1].state
                    ? `<span class="opacity-75">😃 ${data.data.activities[1].state.split("", 25).reduce((o, c) => (o.length === 24 ? `${o}${c}...` : `${o}${c}`), "")}</span>`
                    : ""
                : data.data.activities[1].state
                    ? `<span class="opacity-75">${data.data.activities[1].state.split("", 25).reduce((o, c) => (o.length === 24 ? `${o}${c}...` : `${o}${c}`), "")}</span>`
                    : ""}
...
fetch(url).then(function (response) {
    if (response.ok) {
        ...
        response.json().then(function (data) {
            ...
            var status = `${data.data.active_on_discord_desktop ? data.data.activities[0].emoji.name + ` ${data.data.activities[0].state.split("", 20).reduce((o, c) => (o.length === 19 ? `${o}${c}...` : `${o}${c}`), "")}` : "Anaxes#3274"}`;
            discordStatusHTML.innerHTML += status;
        });
    }
    else {
