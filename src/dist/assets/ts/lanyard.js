...
fetch(url).then(function (response) {
    if (response.ok) {
        ...
        response.json().then(function (data) {
			...
			<div class="flex items-center px-6 py-4 transform hover:scale-105 duration-200 shadow rounded-md px-6 sm:px-4 hover:bg-gray-100 cursor-pointer opacity" style="background-color: #0e141d; border-radius: 1vw; ${data.data.spotify && data.data.activities[1].name === "Spotify"
                ? `background-image: url(${data.data.spotify.album_art_url}); background-position: center; `
                : ""}>
			  <div class="flex items-center">
				<img draggable="false" class="h-16 w-16 rounded-md" src=${data.data.listening_to_spotify &&
                data.data.activities[1].name === "Spotify"
                ? `${"https://i.scdn.co/image/" +
                    `${data.data.activities[1].assets.large_image.split("spotify:")[1]}`}`
                : `https://cdn.discordapp.com/app-assets/${data.data.activities[1].application_id}/${data.data.activities[1].assets.large_image}`} alt="Large Image Icon" />
				...
				<p class="ml-4 text-sm flex flex-col justify-between leading-snug">
					<span class="font-bold">${data.data.listening_to_spotify &&
                data.data.activities[1].name === "Spotify"
                ...
                : `${data.data.activities[1].name === "Visual Studio Code"
                    ...
                    : `${data.data.activities[1].name === "GitHub"
                        ...
                        : "Playing"}`}`}&nbsp;${data.data.activities[1].name}</span>
					<span class="opacity-75">${data.data.listening_to_spotify &&
                data.data.activities[1].name === "Spotify"
                ? `🎶 ${data.data.activities[1].details
                    .split("", 25)
                    .reduce((o, c) => o.length === 24 ? `${o}${c}...` : `${o}${c}`, "")}</span>`
                : `${data.data.activities[1].details
                    .split("", 25)
                    .reduce((o, c) => o.length === 24 ? `${o}${c}...` : `${o}${c}`, "")}</span>`}
...
fetch(url).then(function (response) {
    if (response.ok) {
        ...
        response.json().then(function (data) {
            ...
            var status = `${data.data.active_on_discord_desktop
                ? data.data.activities[0].emoji.name +
                    ` ${data.data.activities[0].state
                        .split("", 20)
                        .reduce((o, c) => o.length === 19 ? `${o}${c}...` : `${o}${c}`, "")}`
                : "Anaxes#3274"}`;
            discordStatusHTML.innerHTML += status;
        });
    }
