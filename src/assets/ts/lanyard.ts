  ...
  if (response.ok) {
    ...
    response.json().then(function (data) {
      ...
      var spotify = `${data.data.listening_to_spotify
          ...
          }" target="_blank" class="hover:text-green-500 flex-shrink-0 flex items-center"><img src=${data.data.spotify.album_art_url
          } class="block h-5 w-auto rounded-full" />&nbsp;<span class="font-bold">${data.data.spotify.song
            .split("", 20)
            .reduce(
              (o: string | any[], c: any) => (o.length === 19 ? `${o}${c}...` : `${o}${c}`),
              ""
            )}</span></a>&nbsp;by&nbsp;<a href="https://open.spotify.com/search/${data.data.spotify.artist.split(";")[0]
          }" target="_blank" class="hover:text-green-500"><span class="font-bold">${data.data.spotify.artist
            .split("", 10)
            .reduce(
              (o: string | any[], c: any) => (o.length === 9 ? `${o}${c}...` : `${o}${c}`),
              ""
          ...
          : data.data.activities[1].name === "GitHub"
            ? `<p><i class="fab fa-github text-white"></i> GitHub:&nbsp;${data.data.activities[1].details
              .split("", 30)
              .reduce(
                (o: string | any[], c: any) => (o.length === 29 ? `${o}${c}...` : `${o}${c}`),
                ""
